...
processAdd :: [String] -> (Name -> String -> String -> String -> PhoneBook -> PhoneBook) -> PhoneBook -> IO PhoneBook
processAdd rest fn book = 
    case rest of 
        [name, pType, code, number] -> do 
            let newState = fn name pType code number book
            putStrLn "Done"
            return  newState
        _ -> do 
...
processFind :: String -> (Name -> PhoneBook -> [Phone]) -> PhoneBook -> IO String
processFind name fn book = 
    case fn name book of 
        [] -> do 
            putStrLn "Not found"
            return "Not found"
        lst -> do 
            --putStrLn $  foldr (\p acc -> show p ++ (if null acc then "" else ", ") ++ acc) "" lst
            let sortedPhones = sort (map show lst)  
            let formattedOutput = intercalate ", " sortedPhones
...
main = do 
    ...
    finalState <- eventLoop Empty -- For grader
    putStrLn finalState